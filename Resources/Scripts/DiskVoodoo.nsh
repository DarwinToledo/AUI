...
Function "MBRID" ; Let's Get the MBRID for OpenSUSE
 ...
 IntFmt $OnlyVal "0x%08X" $OnlyVal ; Convert the value from Decimal to Hexadecimal -was 0x%X fixed for 8 character
 ${StrFilter} "$OnlyVal" "-" "" "" $OnlyVal ; Convert the Hexadecimal value to lower case
 
${If} ${FileExists} "$DestDisk\boot\grub\mbrid" ; see if a mbrid exists
 Rename "$DestDisk\boot\grub\mbrid" "$DestDisk\boot\grub\old-mbrid" ; if so, rename it
${Else}
 CopyFiles "$PLUGINSDIR\mbrid" "$DestDisk\boot\grub\mbrid" ; or create it
${EndIf}

 ${WriteToFile} "$OnlyVal" $R0
FunctionEnd
...
Function WriteToFile
 ...
 FileOpen $R0 '$DestDisk\boot\grub\mbrid' a  ;FileOpen $R0 '$BootDir\multiboot\menu\$Config2Use' a 
 FileSeek $R0 0 END
 FileWrite $R0 '$1'
 FileClose $R0
 Pop $1
